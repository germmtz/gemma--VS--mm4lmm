...
R <- estimGenRel(geno, afs=NULL, thresh=NULL, relationships="additive", method="vanraden1")
...
res <- MMEst(Y=pheno[,"RYT_poids_grains_g_adj"], X = geno.to.test, ZList = list(PAIR = Zpair,  RES=Zres),  VarList =list(PAIR=R, RES=Ires), Method = "Reml")
...
fit <- gemma(model="ulmm", y=y, X=geno.to.test, snp.coords=snp.coords, K.c=R, W=W)
